...
总训练损失由道路功能分类损失 $\mathcal{L}_{\text{cls}}$、蕴含损失 $\mathcal{L}_{\text{CE}}$ 和对比损失 $\mathcal{L}_{\text{CC}}$ 的加权和构成（详见第~\ref{sec:loss} 节）。以下依次阐述各组件的具体设计。
...
局部区域嵌入 $\mathbf{h}_{c_j}$ 由路段嵌入聚合得到。具体地，对每个局部区域 $c_j$（$j = 1, \dots, |\mathcal{C}|$），首先将路段嵌入映射至切空间并执行加权平均，然后通过指数映射投影回流形，并引入\textbf{范数保持因子} $\rho \in (0,1]$ 对空间范数进行重新缩放：
\begin{align}
...
\textbf{R2R（功能区间消息传递）：}功能区之间通过 HGC 交换信息。由于功能区数量较少（$|\mathcal{R}|=30$）且缺乏预定义的拓扑连接，HHRoad 通过功能区嵌入之间的双曲亲和度矩阵动态构建邻接关系：
\begin{equation}
...
\textbf{R2C（功能区向局部区域分发）：}将更新后的功能区嵌入通过赋值矩阵 $\mathbf{A}_{CR}$ 反向分发至局部区域。分发过程在切空间中进行：对每个局部区域 $c_j$，将其所属功能区的嵌入通过对数映射投影至切空间，以赋值权重加权平均后，经指数映射投影回流形（引入范数保持因子 $\rho'$），得到来自功能区的消息向量。随后通过门控双曲插值融合原始局部区域嵌入与功能区消息（具体公式见式~(\ref{eq:f2c_msg})--(\ref{eq:gate_loc})）。

\textbf{C2C（局部区域间消息传递）：}融合了功能区语义的局部区域嵌入在局部区域邻接图上通过 HGC 进一步交换信息：
\begin{equation}
...
\textbf{C2S（局部区域向路段分发）：}更新后的局部区域嵌入通过 $\mathbf{A}_{SC}$ 反向分发至路段，操作方式与 R2C 类似，使用独立的门控参数（具体公式见式~(\ref{eq:c2s_msg})--(\ref{eq:gate_seg})）。

\textbf{S2S（路段间消息传递）：}最后，融合了全部上层语义的路段嵌入在路段邻接图上通过 HGC 进行局部拓扑信息交换：
\begin{equation}
...
上述传播方案中的层次内消息传递（R2R、C2C、S2S）均通过双曲图卷积（Hyperbolic Graph Convolution, HGC）实现。由于 Lorentz 流形是非线性的，标准图卷积无法直接在双曲空间中应用。HGC\cite{chami2019hgcn,liu2019hgnn} 采用\textbf{切空间策略}：将特征映射到原点 $\mathbf{o}$ 处的切空间，在切空间中执行线性邻域聚合，最后将结果映射回流形。具体地，给定双曲节点特征 $\{\mathbf{h}_i\} \subset \mathbb{L}^n$ 和邻接矩阵 $\mathbf{A}$，单层 HGC 包含以下四个步骤：

\paragraph{步骤一：映射至切空间。} 对每个节点 $i$，提取空间分量 $\mathbf{h}_i^s = (h_{i,1}, \dots, h_{i,n})$ 并通过原点处的对数映射计算切空间表示：
\begin{equation}
...
\paragraph{步骤二：邻域聚合。} 在切空间中通过度归一化的消息传递聚合邻居特征：
\begin{equation}
...
\paragraph{步骤三：线性变换。} 对聚合结果施加可学习的线性变换：
\begin{equation}
...
\paragraph{步骤四：映射回流形。} 通过原点处的指数映射将变换后的切向量映射回双曲空间，并重投影以确保满足 Lorentz 约束：
\begin{align}
...
跨层分发步骤（R2C、C2S）中，节点需要融合自身嵌入 $\mathbf{h}$ 和来自父级的消息 $\mathbf{m}$。HHRoad 通过\textbf{门控双曲插值}实现这一融合。以 R2C 步骤为例，对局部区域 $c_j$，首先计算来自功能区的消息向量（引入自顶向下范数保持因子 $\rho' = 0.9$，高于自底向上的 $\rho = 0.8$，因为粗粒度信息在向细粒度传播时应更忠实地保持）：
\begin{equation}
...
HHRoad 的训练由三个互补的损失函数联合驱动。

\subsubsection{道路功能分类损失}

遵循 HRNR 的设置，使用道路功能分类任务作为主要监督信号。给定式~(\ref{eq:output}) 的输出表示 $\mathbf{r}_i$，分类损失采用标准交叉熵：
\begin{equation}
    ...
    \label{eq:loss_cls}
\end{equation}
其中 $\mathcal{B}$ 为训练批次，$y_i$ 为路段 $v_i$ 的真实类别标签，$C$ 为道路类别总数。

\subsubsection{蕴含锥损失}

为在双曲空间中显式编码层次包含关系，HHRoad 引入\textbf{蕴含锥}机制\cite{ganea2018hyperbolic}。在双曲空间中任意一点 $\mathbf{x} \in \mathbb{L}^n$，蕴含锥由半孔径角定义：
\begin{equation}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
    \item \textbf{功能区 $\Rightarrow$ 局部区域}：对每个功能区 $r_k$ 及其下属局部区域 $c_j$；
    \item \textbf{局部区域 $\Rightarrow$ 路段}：对每个局部区域 $c_j$ 及其下属路段 $v_i$；
    \item \textbf{路段 $\Leftrightarrow$ 路段}：对每对拓扑相邻的路段 $(v_i, v_j) \in \mathcal{E}$。
\end{enumerate}
...
\subsubsection{层次对比损失}

为进一步锐化嵌入空间的语义结构，HHRoad 在两个粒度上施加对比学习。

\paragraph{路段层内对比学习。} 对每条采样边 $(v_i, v_j) \in \mathcal{E}$，以路段 $v_i$ 为锚点，拓扑相邻路段 $v_j$ 为正例，随机采样 $K$ 条路段为负例。相似度定义为 Lorentz 距离的负值，损失函数采用 InfoNCE 形式：
\begin{equation}
...
\paragraph{跨层对比学习。} 对每个局部区域 $c_j$，将其与随机选取的下属路段 $v_i \in c_j$（正例）和 $K$ 条随机路段（负例）配对：
\begin{equation}
